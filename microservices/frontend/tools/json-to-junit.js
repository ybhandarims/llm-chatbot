...
function toJUnit(jsonPath, outPath) {
  ...
  for (const e of entries) {
    ...
    if (status !== "pass" && status !== "ok" && status !== "success") {
      failures += 1;
      const msg =
        (e.error && (e.error.message || e.error)) || e.reason || "failed";
...
if (require.main === module) {
  ...
  const jsonPath =
    args[0] || path.join(process.cwd(), "reports", "frontend-tests.json");
  const outPath =
    args[1] || path.join(process.cwd(), "reports", "frontend-tests.xml");
