...
function makeMockFetch() {
  return (url, options) => {
    ...
    if (u.endsWith('/chat/send')) {
      return Promise.resolve({ ok: true, json: async () => ({ conversation: { id: 'conv1', messages: [{ role: 'assistant', content: 'Echo' }] } }) });
    }
...
test('app bootstrap loads UI and renders conversations/messages', async () => {
  ...
  assert.ok(status === 'Ready' || status === 'New chat ready' || status === 'System prompt updated');

  const convoList = dom.window.document.getElementById('conversation-list');
  assert.ok(convoList.children.length >= 1, 'Conversation list should render at least one item');

  const messages = dom.window.document.getElementById('messages');
  assert.ok(messages.children.length >= 1, 'Messages should render at least one message');
});
